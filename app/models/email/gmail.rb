...
class Email::Gmail
  class ReauthNeeded < StandardError; end

  BLACKLIST_SUBJECTS = [ "Login", "Payment Received" ].map(&:downcase).freeze
  ...
  class << self
    def ingest_latest(acct, senders:, days: 2, unread_only: true, max: 200)
      ...
      list.messages.each do |m|
        ...
        text, html = extract_html_text(msg.payload&.parts || [])
    ...
    def headers_hash(headers)
      (headers || []).to_h { |h| [ h.name, h.value ] }
    end

    def extract_html_text(parts)
      parts ||= []
